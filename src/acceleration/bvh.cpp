...
radiance::acceleration::BVHNode::BVHNode(geometry::HitList &&hitList):
    BVHNode(hitList.objects)
{
}
...
radiance::acceleration::BVHNode::BVHNode(std::vector<std::shared_ptr<radiance::geometry::Hittable>> &objects, size_t start, size_t end)
{
...
bool radiance::acceleration::BVHNode::boundingBox(geometry::AABB &outBox) const
{
    outBox = box;
    return true;
}
...
{
    geometry::AABB left_box,right_box{};
    lhs->boundingBox(left_box);
    rhs->boundingBox(right_box);
